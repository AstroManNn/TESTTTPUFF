...
// 1.A Добавить товар (JSON, без multipart) — фикс проблем с кодировкой текстовых полей
// Используется фронтом: сначала создаем товар текстом (UTF-8 JSON), затем при наличии фото загружаем фото отдельным запросом.
app.post('/api/admin/product/create', async (req, res) => {
    try {
        ...
        const ins = await pool.query(
            'INSERT INTO products (name, category, description, price, purchase_price, stock, image_url) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id',
            [name, category, description || '', price, purchase_price || 0, stock || 0, defaultImage]
        );
